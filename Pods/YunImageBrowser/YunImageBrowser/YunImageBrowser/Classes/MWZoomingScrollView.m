...
- (void)displayImageFailure {
    ...
    if (![_photo respondsToSelector:@selector(emptyImage)] || !_photo.emptyImage) {
        if (!_loadingError) {
            ...
            _loadingError.autoresizingMask =
                    UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleTopMargin |
                    UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleRightMargin;
...
- (CGFloat)initialZoomScaleWithMinScale {
    ...
    if (_photoImageView && _photoBrowser.zoomPhotosToFill) {
        ...
        CGFloat
                xScale =
                boundsSize.width / imageSize.width;    // the scale needed to perfectly fit the image width-wise
        CGFloat
                yScale =
                boundsSize.height / imageSize.height;  // the scale needed to perfectly fit the image height-wise
...
- (void)setMaxMinZoomScalesForCurrentBounds {
    ...
    CGFloat
            minScale =
            MIN(xScale, yScale);                 // use minimum of these to allow the image to become fully visible
...
- (void)layoutSubviews {
    ...
    if (!_loadingIndicator.hidden) {
        _loadingIndicator.frame = CGRectMake(floorf((self.bounds.size.width - _loadingIndicator.frame.size.width) / 2.),
                                             floorf((self.bounds.size.height - _loadingIndicator.frame.size.height) /
                                                    2),
